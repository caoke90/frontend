...
function getactiveEntry() {
  const arr = glob.sync(__dirname+'/../src/entry/active/**/*.html')

  const active={}
  arr.forEach(function (file) {
    file.replace(/entry\/active\/(.+)\.html$/,function (m,p1) {
      active[p1]=file
    })
  })
...
function getdevHtmlWebpack() {
  ...
  for(let name in active){
    htmlConfig.push(new HtmlWebpackPlugin({
      title:productName,
      filename: productName+'/active/'+name+'.html',
      template: __dirname+'/../src/entry/active/'+name+'.html',
      inject: false
    }))
  }
  return htmlConfig;
